...
except: import scipy.fftpack as fft
import scipy.signal as signal
import glob
import matplotlib.pyplot as pl
from scipy.ndimage import gaussian_filter, median_filter, binary_opening, binary_closing, binary_dilation, label, laplace
...
class RetinotopicMap():
    ...
    def __init__(self, savefolder, savename, sigma_p, sigma_s, s_method, sigma_t, sigma_c, openIter, closeIter, dilateIter, shiftPhase, borderWidth, epsilon, rotateMap):
    ...
    def _load_data(self):
        ...
        self.ref = exposure.equalize_adapthist(ref, clip_limit=0.03, kernel_size=(int(ref.shape[0]/20), int(ref.shape[1]/20)))
    ...
    def _get_visual_border(self):
        ...
        self.vis_borders = binary_dilation(binary_opening(binary_closing(np.abs(self.sign_thresh), iterations=self.closeIter), 
    ...
    def plot(self):
        ...
        if self.s_method == 'gaussian':
            self.ax[0,1].imshow(rotate_image(gaussian_filter(self.sign, sigma=self.sigma_s), self.rotateMap), cmap=pl.cm.jet)
        elif self.s_method == 'median':
            self.ax[0,1].imshow(rotate_image(median_filter(self.sign, int(self.sigma_s)), self.rotateMap), cmap=pl.cm.jet)
        self.ax[0,1].set_title('Sign map')

        # Plot reference map (Zhuang et al. 2017, Figure 3C)
        try: reference = pl.imread(r'C:\Users\TrenholmLab\TrenholmLab_Analysis_Scripts\Retinotopic_Mapping\Reference_Zhuang_2017_Fig3.png')
        except: reference = np.zeros_like(self.sign)
        ...
        azi_contour = self.ax[1,0].contourf(rotate_image(azimuth_map, self.rotateMap), cmap=pl.cm.jet, levels=10, zorder=-1)
...
class PhaseMap():
    ...
    def _get_phase_map(self, fn, angular_dir):
        ...
        data_ds = load_data(fn, start_frame=start_frame, end_frame=nimagtot)
